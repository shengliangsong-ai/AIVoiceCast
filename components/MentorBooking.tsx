...
import { Calendar, Clock, User, ArrowLeft, Search, Briefcase, Sparkles, CheckCircle, X, Loader2, Play, Users, Mail, Video, Mic, FileText, Download, Trash2, Monitor, UserPlus, Grid, List, ArrowDown, ArrowUp, Heart, Share2, Info, ShieldAlert, ChevronRight, Coins, Check as CheckIcon, HeartHandshake, Edit3, Timer, Coffee, Sunrise, Sun, Sunset, Hash, Star, ShieldCheck, MoreHorizontal, Zap, Bot } from 'lucide-react';
...
const SafeAvatar = ({ src, name, icon: Icon = User }: { src?: string, name: string, icon?: any }) => {
    // Strictly filter out third-party dynamic image services and placeholders
    ...
    if (hasValidSrc) {
        return <img src={src} className="w-10 h-10 rounded-xl object-cover border-2 border-slate-800 shadow-lg" alt={name} />;
    }
    
    return (
        <div className="w-10 h-10 rounded-xl bg-slate-800 flex flex-col items-center justify-center text-slate-400 border-2 border-slate-800 shadow-lg overflow-hidden relative group">
            <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 to-purple-500/10 opacity-0 group-hover:opacity-100 transition-opacity" />
            {initials ? (
                <span className="text-[10px] font-black text-indigo-400 relative z-10">{initials}</span>
            ) : (
...
export const MentorBooking: React.FC<MentorBookingProps> = ({ currentUser, userProfile, channels, onStartLiveSession }) => {
  ...
  const filteredAiMentors = useMemo(() => {
      // Filter for mentors with high engagement or specific specialized tags
      const popular = channels.filter(c => c.likes > 5 || c.tags.includes('Tech') || c.tags.includes('Engineering'));
      if (!searchQuery.trim()) return popular;
      const q = searchQuery.toLowerCase();
      return popular.filter(c => c.title.toLowerCase().includes(q) || c.description.toLowerCase().includes(q));
  ...
  const handleOpenAiBooking = (m: Channel) => {
      setSelectedMentor(m);
  ...
  const handleBookSession = async () => {
    ...
    try {
        const newBooking: Booking = {
            ...
            mentorImage: isP2P ? (bookingMember!.photoURL || '') : selectedMentor!.imageUrl,
        ...
        alert(isP2P ? "Request sent! Notification emails dispatched." : "AI session booked! Confirmation sent.");
  ...
  if (bookingMember || selectedMentor) {
      ...
      const avatarSrc = bookingMember ? (bookingMember.photoURL || '') : selectedMentor!.imageUrl;
      const avatarName = bookingMember ? (isSelf ? 'My Availability (Live)' : bookingMember.displayName) : selectedMentor!.title;

      return (
        <div className="max-w-5xl mx-auto my-8 animate-fade-in-up px-4 sm:px-6">
            <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] shadow-2xl overflow-hidden">
                <div className="p-8 border-b border-slate-800 flex items-center gap-6 bg-slate-950/50">
                    ...
                    <SafeAvatar src={avatarSrc} name={avatarName} />
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3">
                            <h2 className="text-2xl sm:text-3xl font-black text-white italic tracking-tighter uppercase truncate">{avatarName}</h2>
                            {isSelf && <span className="bg-emerald-600 text-white text-[10px] font-black border border-emerald-500/50 px-2 py-0.5 rounded-full uppercase shadow-lg shadow-emerald-900/40 whitespace-nowrap">Synced</span>}
                        </div>
                                    ...
                                    <div className="text-xs text-slate-500 italic p-12 text-center border-2 border-dashed border-slate-800 rounded-[2rem] bg-slate-900/20">
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <div className="mt-4 p-4 bg-slate-950 rounded-2xl border border-slate-800">
                                                <p className="text-[10px] font-black uppercase text-indigo-400 tracking-widest mb-1">Current availability:</p>
                        ...
                        <div className="lg:col-span-5 space-y-8">
                            <div className="bg-slate-950 p-8 rounded-[2rem] border border-slate-800 shadow-inner space-y-6">
                                ...
                                <textarea 
                                    ...
                                    placeholder="What technical problems or goals should we address?"
                                />
                                ...
                                {bookingMember && !isSelf && (
                                    <div className="bg-amber-900/10 border border-amber-500/20 p-5 rounded-2xl flex items-center gap-4 animate-fade-in">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-sm font-bold text-white">Honorarium: 50 Coins</p>
                                ...
                                <div className="space-y-4 pt-4">
                                    <button 
                                        ...
                                        className="w-full py-5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-indigo-900/40 transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-30 disabled:grayscale"
                                    >
                                        {isBooking ? <Loader2 className="animate-spin mx-auto" size={24} /> : isSelf ? 'Lock Personal Focus Slot' : 'Authorize & Book Session'}
                                    </button>
                                </div>
                            </div>
                        </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 animate-fade-in space-y-10 flex flex-col h-full overflow-hidden">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 shrink-0">
            <div>
                ...
                <p className="text-slate-400 font-medium max-w-xl">Accelerate your growth. Connect with domain experts or leverage specialized AI personas for technical guidance.</p>
            </div>
        ...
        {activeTab === 'members' && (
            <div className="flex-1 flex flex-col min-h-0 space-y-6">
                <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] shadow-xl flex flex-col md:flex-row items-center gap-8 relative overflow-hidden group shrink-0">
                    ...
                    <div className="flex-1 text-center md:text-left relative z-10">
                        ...
                        <p className="text-slate-400 text-sm leading-relaxed max-w-2xl">Connect with verified experts and earn VoiceCoins by sharing your unique expertise.</p>
                        {userProfile && (
                            <button onClick={() => handleOpenBooking(userProfile)} className="mt-4 px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-black uppercase tracking-widest rounded-xl shadow-lg transition-all active:scale-95">Set My Availability</button>
                        )}
                    </div>
                </div>
                ...
                <div className="relative shrink-0">
                    ...
                    <input type="text" placeholder="Search experts by name or skill..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-2xl pl-12 pr-6 py-4 text-white focus:ring-2 focus:ring-indigo-500 outline-none shadow-inner"/>
                </div>

                <div className="flex-1 bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden shadow-2xl flex flex-col min-h-0">
                    <div className="overflow-x-auto h-full scrollbar-thin scrollbar-thumb-slate-700">
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-slate-950/50 sticky top-0 z-20 backdrop-blur-md border-b border-slate-800">
                                <tr>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-500 tracking-widest">Mentor</th>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-500 tracking-widest">Expertise</th>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-500 tracking-widest hidden md:table-cell">Status</th>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-500 tracking-widest">Honorarium</th>
                                ...
                                {loadingMembers ? (
                                    <tr>
                                        <td colSpan={5} className="py-20 text-center">
                                            <Loader2 className="animate-spin mx-auto text-indigo-400" size={40}/>
                                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mt-4">Paging Directory...</p>
                                        </td>
                                    </tr>
                                ) : filteredMembers.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="py-20 text-center text-slate-500 italic font-medium">No matching experts found.</td>
                                    </tr>
                                ) : filteredMembers.map(m => (
                                    <tr key={m.uid} className="group hover:bg-slate-800/30 transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center gap-3">
                                                <div className="relative">
                                                    <SafeAvatar src={m.photoURL} name={m.displayName} />
                                                    <div className="absolute -bottom-1 -right-1 bg-emerald-500 w-3 h-3 rounded-full border-2 border-slate-900"></div>
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-white group-hover:text-indigo-400 transition-colors">@{m.displayName}</p>
                                                    <p className="text-[9px] text-slate-600 font-mono">UID: {m.uid.substring(0,8)}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex flex-wrap gap-1.5 max-w-[200px] sm:max-w-xs overflow-hidden">
                                                {(m.interests || ['General AI']).slice(0,3).map(i => (
                                                    <span key={i} className="text-[8px] font-black uppercase bg-slate-950 text-slate-500 px-2 py-0.5 rounded-md border border-slate-800 group-hover:border-indigo-500/30 transition-colors">#{i}</span>
                                                ))}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 hidden md:table-cell">
                                            <div className="flex items-center gap-1.5 px-2 py-1 bg-emerald-900/20 text-emerald-400 border border-emerald-500/20 rounded-lg w-fit">
                                                <ShieldCheck size={12}/>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <button 
                                                ...
                                                className="px-4 py-2 bg-indigo-600/10 hover:bg-indigo-600 text-indigo-400 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border border-indigo-500/20 hover:border-indigo-500 shadow-lg active:scale-95 flex items-center gap-2 ml-auto"
                                            >
        ...
        {activeTab === 'ai_mentors' && (
            <div className="flex-1 flex flex-col min-h-0 space-y-6">
                <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] shadow-xl flex flex-col md:flex-row items-center gap-8 relative overflow-hidden group shrink-0">
                    <div className="absolute top-0 right-0 p-12 bg-purple-500/10 blur-[80px] rounded-full group-hover:bg-purple-500/20 transition-all"></div>
                    <div className="p-6 bg-purple-950/40 rounded-[2rem] border border-purple-500/30 text-purple-400 shrink-0"><Bot size={48} /></div>
                    <div className="flex-1 text-center md:text-left relative z-10">
                        <h3 className="text-xl font-bold text-white mb-2">High-Intelligence Personas</h3>
                        <p className="text-slate-400 text-sm leading-relaxed max-w-2xl">Leverage specialized neural nodes to simulate technical interviews, design sessions, and expert knowledge retrieval.</p>
                    </div>
                </div>

                <div className="relative shrink-0">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18}/>
                    <input type="text" placeholder="Search AI personas..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-2xl pl-12 pr-6 py-4 text-white focus:ring-2 focus:ring-indigo-500 outline-none shadow-inner"/>
                </div>
                ...
                <div className="flex-1 bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden shadow-2xl flex flex-col min-h-0">
                    <div className="overflow-x-auto h-full scrollbar-thin scrollbar-thumb-slate-700">
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-slate-950/50 sticky top-0 z-20 backdrop-blur-md border-b border-slate-800">
                                <tr>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-500 tracking-widest">AI Agent</th>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-500 tracking-widest">Specialization</th>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-500 tracking-widest hidden md:table-cell">Engagement</th>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase text-slate-500 tracking-widest">Engine</th>
                                    <th className="px-6 py-4 text-right text-[10px] font-black uppercase text-slate-500 tracking-widest">Action</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-800/50">
                                {filteredAiMentors.map(m => (
                                    <tr key={m.id} className="group hover:bg-slate-800/30 transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center gap-3">
                                                <SafeAvatar src={m.imageUrl} name={m.title} icon={Bot} />
                                                <div>
                                                    <p className="text-sm font-bold text-white group-hover:text-indigo-400 transition-colors">{m.title}</p>
                                                    <p className="text-[9px] text-slate-600 font-mono uppercase">ID: {m.id.substring(0,8)}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="text-xs text-slate-400 line-clamp-1 max-w-xs">{m.description}</p>
                                        </td>
                                        <td className="px-6 py-4 hidden md:table-cell">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2 px-2 py-1 bg-indigo-900/20 text-indigo-400 border border-indigo-500/20 rounded-lg w-fit">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <button 
                                                onClick={() => handleOpenAiBooking(m)} 
                                                className="px-4 py-2 bg-purple-600/10 hover:bg-purple-600 text-purple-400 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border border-purple-500/20 hover:border-purple-500 shadow-lg active:scale-95 flex items-center gap-2 ml-auto"
                                            >
                                                Consult AI <ChevronRight size={14}/>
                                            </button>
                                        </td>
                                    </tr>
                                ))}
        ...
        {activeTab === 'my_bookings' && (
            <div className="max-w-4xl mx-auto w-full flex-1 overflow-y-auto pr-2 scrollbar-thin space-y-4">
                {isLoadingBookings ? <div className="py-20 text-center"><Loader2 className="animate-spin mx-auto text-indigo-400" size={48}/></div> : myBookings.length === 0 ? (
                    <div className="py-32 text-center text-slate-500 bg-slate-900/30 border-2 border-dashed border-slate-800 rounded-[3rem] space-y-6">
                        ...
                        <button onClick={() => setActiveTab('members')} className="text-indigo-400 font-bold uppercase tracking-widest text-xs hover:underline">Browse Experts</button>
                    </div>
                ) : (
                    myBookings.map(b => (
                        <div key={b.id} className="bg-slate-900 border border-slate-800 p-6 rounded-[2rem] flex flex-col md:flex-row items-center gap-6 group hover:border-indigo-500/30 transition-all shadow-xl animate-fade-in-up">
                            <div className="bg-slate-950 p-5 rounded-[1.5rem] text-center min-w-[120px] border border-slate-800 shadow-inner">
                                <span className="block text-2xl font-black text-white tabular-nums">{b.time}</span>
                            ...
                            <div className="flex gap-2">
                                {b.status === 'scheduled' ? (
                                    ...
                                    <div className="text-xs text-slate-500 font-bold italic px-4 bg-slate-800/50 py-2 rounded-lg flex items-center">Pending Acceptance...</div>
                                )}
